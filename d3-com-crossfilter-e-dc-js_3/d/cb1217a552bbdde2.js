...
d3.csv("https://gist.githubusercontent.com/emanueles/d8df8d875edda71aa2e2365fae2ce225/raw/1e949d3da02ed6caa21fe3a7a12a4e5a611a4bab/stocks.csv").then(function(data){
  ...
  let face0 = data[0].facebook
  let google0 = data[0].google
  data.forEach(function(d,i){
       d.date = parseDate(d.date)
       d.google = +d.google
       d.facebook = +d.facebook
       d.facebookPer = 100*(d.facebook/face0 - 1)  
       d.googlePer = 100*(d.google/google0 - 1)
  ...
  main.variable(observer("buildcomposite")).define("buildcomposite", ["md","container","dc","d3","dateDim","googleByDayGroup","fbByDayGroup"], function(md,container,dc,d3,dateDim,googleByDayGroup,fbByDayGroup)
{
  ...
  main.variable(observer("googlePercent")).define("googlePercent", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.googlePer)
)});
  main.variable(observer("facebookPercent")).define("facebookPercent", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.facebookPer)
)});
  main.variable(observer("buildcomposite2")).define("buildcomposite2", ["md","container","dc","d3","dateDim","googlePercent","facebookPercent"], function(md,container,dc,d3,dateDim,googlePercent,facebookPercent)
{
  let view = md`${container('chart2', 'Valores das ações do Google e do Facebook')}`
  let compositeChart = dc.compositeChart(view.querySelector("#chart2"))
  let xScale = d3.scaleTime()
  ...
  compositeChart.width(800)
              ...
              .compose([
                  dc.lineChart(compositeChart)
                    .group(googlePercent, 'Google')
                    .ordinalColors(['steelblue']),
                  dc.lineChart(compositeChart)
                    .group(facebookPercent, 'Facebook')
                    .ordinalColors(['darkorange'])])
  dc.renderAll()
  return view      
}
...
d3.json("https://raw.githubusercontent.com/emanueles/datavis-course/master/assets/files/observable/movies.json")
  ...
  main.variable(observer("crossData2")).define("crossData2", ["crossfilter","dataset2"], function(crossfilter,dataset2){return(
crossfilter(dataset2)
)});
  main.variable(observer("dimDataYear")).define("dimDataYear", ["crossData2"], function(crossData2){return(
crossData2.dimension(d => d.Year)
)});
  main.variable(observer("dimDataGenre")).define("dimDataGenre", ["crossData2"], function(crossData2){return(
crossData2.dimension(d => d.Genre)
)});
  main.variable(observer("bilheteriaYaer")).define("bilheteriaYaer", ["dimDataYear"], function(dimDataYear){return(
dimDataYear.group().reduceSum(d => d. Worldwide_Gross_M)
)});
  main.variable(observer("bilheteriaGenre")).define("bilheteriaGenre", ["dimDataGenre"], function(dimDataGenre){return(
dimDataGenre.group().reduceSum(d => d. Worldwide_Gross_M)
)});
  main.variable(observer("byYear")).define("byYear", ["md","container","dc","dimDataYear","bilheteriaYaer","d3"], function(md,container,dc,dimDataYear,bilheteriaYaer,d3)
{
  let view = md`${container('chart3', 'Bilheterias por ano')}`
  let barChart = dc.barChart(view.querySelector("#chart3"))
  barChart.width(800)
          .height(400)
          .dimension(dimDataYear)
          .group(bilheteriaYaer)
          .x(d3.scaleBand())
  ...
  main.variable(observer("byGenrer")).define("byGenrer", ["md","container","dc","dimDataGenre","bilheteriaGenre","d3"], function(md,container,dc,dimDataGenre,bilheteriaGenre,d3)
{
  let view = md`${container('chart4', 'Bilheterias por genero')}`
  let barChart = dc.barChart(view.querySelector("#chart4"))
  barChart.width(800)
          .height(400)
          .dimension(dimDataGenre)
          .group(bilheteriaGenre)
          .x(d3.scaleBand())
          .xUnits(dc.units.ordinal)
          .barPadding(0.5)
          .outerPadding(0.2)
