...
function zoomToFeature(e) {
  ...
  main.variable(observer("info")).define("info", ["L","homicidesByNamePopulation","map"], function(L,homicidesByNamePopulation,map)
{
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Número de homicídios</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br />' + homicidesByNamePopulation.get(feat.properties.NOME) + ' homicídios'
...
d3.csv("https://gist.githubusercontent.com/emanueles/bae10fc42b13886b5a00b79737f2b50d/raw/ffb35abe6d9ba0c4ce4866b827f1916748ab65ef/homicidios_2012.csv").then(function(data) {
  ...
  main.variable(observer("homicidesByNamePopulation")).define("homicidesByNamePopulation", ["d3"], function(d3){return(
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
  let bairrosMap2 = d3.map()
  data.forEach(function(d) {
    d.crimesPorCem = (d.Homicidios/d.Populacao) * 100000
    bairrosMap2.set(d.Bairro,+d.crimesPorCem)
  })
  return bairrosMap2
})
)});
  main.variable(observer("style")).define("style", ["colorScale","homicidesByNamePopulation"], function(colorScale,homicidesByNamePopulation){return(
function style(feature) {
		 return {
					...
					dashArray: '3',
					fillOpacity: 0.6,
					fillColor: colorScale(homicidesByNamePopulation.get(feature.properties.NOME))
				};
	}
)});
